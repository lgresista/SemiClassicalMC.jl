...
function measure!(
    ...
    msx = mean(norm(m[[6, 7, 10, 11, 14, 15]]))
    push!(obs.msx, msx)
    push!(obs.binder_msx, msx.^2, msx.^4)
    msz = mean(norm(m[[8, 12, 16]]))
    push!(obs.msz, msz)
