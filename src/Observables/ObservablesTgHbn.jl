...
function measure!(
    ...
    msx = mean(norm(m[[7, 8, 10, 11, 13, 14]]))
    push!(obs.msx, msx)
    push!(obs.binder_msx, msx.^2, msx.^4)
    msz = mean(norm(m[[9, 12, 15]]))
    push!(obs.msz, msz)
